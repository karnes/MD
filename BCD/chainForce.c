...
/* loop over all bond stretches*/
	...
	for (i=1;i<nCarbon-2;i++){
		n = fa+i;
    		bond_e += calcstrch(n,n+1,1);/*CH2-CH2 bonds*/
	}
    	bond_e += calcstrch(fa+nCarbon-2,fa+nCarbon-1,0);/*CH3-CH2 bond*/
/* loop over all torsions*/
	tors_e=0.;
	for (i=0;i<nCarbon-3;i++){
		n = fa+i;
    		tors_e += calctorq(n,n+1,n+2,n+3,i);
	}
/* loop over all angle bends*/
	bend_e = 0.;
	for (i=0;i<nCarbon-2;i++){
		n = fa+i;
...
/* total intramolecular potential energy*/
    	CHAINV += bond_e + tors_e + bend_e + nb_e;
}
...
da = acos(n1n2) - eqBend;
pe = 0.5*kBend*da*da;
dedd = kBend*da/ (sqrt(1.-n1n2*n1n2)*r1*r2);
