...
/*	Get the appropriate input paramters for the Chloroform intra- forces */
	...
	fgets(sbuf,256,fp);
	if (sscanf(sbuf,"%lf%lf%lf",&R_CCl, &R_CH, &gama) != 3){
	...
	ks[1] = ks[2] = ks[3] = k_CCl*convF;
	ks[4] = k_CH*convF;
	kb[1][2] = kb[1][3] = kb[2][3] = k_ClCCl*R_CCl*R_CCl*convF;
	kb[1][4] = kb[2][4] = kb[3][4] = k_ClCH*R_CH*R_CH*convF;
	/* calculte the pair equilibrium distances matrix*/
	gamH = PI- asin(sin(gama*PI/360.0)*2.0/sqrt(3.0));/*The H-C-Cl angle*/
...
//	   QCFM[4] = 0.10;
	   ...
	   fprintf(stderr,"ssyInit.c: Problem with input file. unidentified sigH. sigH = %f\n",sigH);
	   exit(0);
	}
