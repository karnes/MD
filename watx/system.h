...
int	tdpoint;
//int	neqFlag;
int	k10flag;		/* "kill at 10" flag -- ends trajectory if any water O z > 10 */
...
double VINTx;		/* total interaction with water zero */
...
tripd watx;  // center of mass of water zero

double kfnit; // 'killfinger'/flat interface constraint  potential 
double kfwat; //  applied to nitrobenzene or water molecules           
double rOO, ADmin; // transferring water: 'last bond' acceptor (0) or donor (1) ?
int AorD;
#define r2watwat (3.40*3.40) // distance for water to be in water's hydration shell
...
int     Nshel,Nshel2; /*number of water molecules in first shell*/
...
/* for snapshots */
/* snap is a z position tracker; taking sequential snapshots, low to high z in window. interval = snap_w */
double snap;
#define snap_w 0.5
// (to generate TS snapshots)
char fileHead[80];
int fileVers;

int cMer;
double Umer,Umer0,Umer1;
