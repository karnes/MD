...
/*****	Determine image vector for OXYGEN i - OXYGEN j.  ******/
		...
		r2OO = r2 = delfx*delfx + delfy*delfy + delfz*delfz;
		if	(r2 >= swr2max)
			continue;
		if	(r2 <= swr2min)
			{
...
//					dedr = watts(r2,addr[m][n],&eg);
					...
					dedr = spc(r2,m,n,&eg,&wc,k/*waterIndex*/);
				}
				else {
...
/*****	Resolve forces on atoms.  ******/
		...
		if(fixShel>0){
		  if(i==0 || j==0){ //||i==fixw[0]||i==fixw[1]){
		     if(i==fixw[0]||i==fixw[0]||j==fixw[1]||j==fixw[1]){
		        Umer+=0.0;
			cMer++;
		     }
		     else{
		  ...
		  else if(i==fixw[0] || j==fixw[0]){ //||i==fixw[0]||i==fixw[1]){
		     if(i==0||j==0||i==fixw[1]||j==fixw[1]){
		        Umer+=0.0;
			cMer++;
		     }
		     else{
		  ...
		  else if(i==fixw[1] || j==fixw[1]){ //||i==fixw[0]||i==fixw[1]){
		     if(i==fixw[0]||j==fixw[0]||i==0||j==0){
		        Umer+=0.0;
			cMer++;
		     }
		     else{
