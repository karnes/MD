...
/* read in window parameters */	
	...
	if (sscanf(sbuf,"%lf%lf%lf%lf",&center_w,&width_w,&power_w,&pot_w)!=4) {
        ...
        if (width_w > 0. && width_w < 10.){
                fprintf(stderr,"sysInit.c: water 0 constrained between ");
                fprintf(stderr,"%f and %f ",center_w-width_w/2.,center_w+width_w/2.);
                fprintf(stderr,"relative to system CoM\n");
        }
	fgets(sbuf,256,fp);
...
/* read potential biasing, KillFinger flags */  
	...
	if(biasingOn == 1){
	   fgets(sbuf,256,fp);
           if(sscanf(sbuf,"%lf %lf %lf %lf %lf",&bias_a,&bias_b,&bias_c,&bias_d,&bias_gs) !=5){
		fprintf(stderr,"sysinit: error reading biasing parameters. Exiting.\n");
		exit(0);
	   }
	   fprintf(stderr,"sysInit.c: biasing: a = %f, b = %f, c = %f, d = %f, gs = %f\n",bias_a,bias_b,bias_c,bias_d,bias_gs); 
        }
