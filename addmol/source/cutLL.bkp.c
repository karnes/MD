/*
 *	cutLL.c:reads in water/BrO - BCD binary file
 *			chops xwall and ywall to user-defined lengths
 *
 *	To call:	cutll <watBrO binary> <output .as> <x,y-box edge (Å)> <nBrO, bin input> <nBCD>
 *
 */
#include "typedefs.h"
#include "atomtypes.h"
 ...
 */
	if (argc != 6) {
		fprintf(stderr, "usage: %s <l/l binary> <output .as> <x,y-box edge (Å)> <nBrO in bin input> <nBCD in bin input>\n",
			argv[0]);
		exit(0);
	}
...
nw = (natoms - nBrO*BrOs - nBCD*BCDs)/3;/* number of water molecules in the original file*/
/* 
 * remove all water molecules with image center outside +/- 22.5 Å
 */
nWR = 0;
for(i=0;i<nw;i++){
   k = i*3;
...
for(i=0;i<nBrO;i++){
   ...
   if(fabs(pos[k].fx)>(edge/2.0) || fabs(pos[k].fy)>(edge/2.0)){
      nBR++;
      inB[i] = 1; // remove this Br-Octane
   }
}  


fprintf(stderr," %d Br-octane molecules outside of new box\n",nBR);
...
for (j=0; j<nBrO; j++) {/*loop over all the Br-oct molecules*/
   if (inB[j] == 0){/*if this Br-oct is in the box; print it*/
	...
	for(k=0;k<BrOs;k++){
	   fprintf(fpo,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",
		i,atom[l+k].type,pos[l+k].fx,pos[l+k].fy,pos[l+k].fz,atom[l+k].flags,3*(nw-nWR-rmxW)+((i-(3*(nw-nWR-rmxW)))/BrOs)*BrOs,atom[l+k].param1,atom[l+k].param2);
	   i++;
	}
   }
}
/*print the BCD*/
//i = 0; /*maintain same counter...*/
for (j=0; j<nBCD; j++) {/*loop over BCD molecule*/
	l = nw*3+BrOs*nBrO;
	for(k=0;k<BCDs;k++){
	   fprintf(fpo,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",
		i,atom[l+k].type,pos[l+k].fx,pos[l+k].fy,pos[l+k].fz,atom[l+k].flags,atom[l+k].parent - 3*(nWR+rmxW) - BrOs*(nBR+rmxB),atom[l+k].param1,atom[l+k].param2);
	   i++;
	}
}
