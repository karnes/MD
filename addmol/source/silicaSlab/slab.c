/*
 *	slab.c:	This program creates an ascii text file for a silica
 *			[beta cristobalite]  slab surface.
 *
 ...
 *
 *	To call:	slab <#unit cels in x>  <#of unit cells in y> 
					<#unit cells in z> <ascii_file>
 *
 */
#include	<stdlib.h>
#include	<stdio.h>
...
main ( int argc, char *argv[] )
{
	if (argc != 5)
	{
		printf("usage: %s <# unit cells in x> <# unit cells in y> <# unit cells in z> <ascii_file>\n", argv[0]);
		exit(0);
	}

	int cellsx = atoi(argv[1]);
	int cellsy = atoi(argv[2]);
...
/*	Set dummy data for header   */
	...
	int xtrInQ = 0;
	int flag = A_MAJOR | A_DRAW | A_MINOR;
	...
	struct atom A = {113, edge*0.788675, edge*0.788675, edge*0.15470054, flag, 2, 0, 1};//A
	uCell[0] = A;
	struct atom B = {113, edge*0.0, edge*1.0, edge*0.0, flag, 2, 0, 1};//B
	uCell[1] = B;
	struct atom C = {113, edge*1.0, edge*0.0, edge*0.0, flag, 2, 0, 1};//C
	uCell[2] = C;
	struct atom D = {113, edge*0.3660254, 0.3660254, 0, flag, 2, 0, 1};//D
	uCell[3] = D;
	struct atom E = {113, edge*0.78867513, edge*0.78867513, edge*0.28867513, flag, 2, 0, 1};//E
	uCell[4] = E;
	
	double ax = 0.78867513;
	double ay = 0.78867513;
	
	struct atom a = {0, edge*ax, edge*ay, edge*0.72168784, flag, 2, 0, 1};//a or a'
	uCell[5] = a;
	struct atom b = {0, edge*0.44722285, edge*0.94722285, edge*0.038567, flag, 2, 0, 1};//b
	uCell[6] = b;
	struct atom c = {0, edge*0.84145228, edge*0.34145228, edge*0.25010814, flag, 2, 0, 1};//c
	uCell[7] = c;
	struct atom d = {0, edge*0.16895027, edge*0.98575027, edge*0.14433757, flag, 2, 0, 1};//d
	uCell[8] = d;
