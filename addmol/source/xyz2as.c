...
#define atoms (1+molec*per+waters*3)
#define waters 3236
#define molec 4
#define per 19 
#define zUp 0.0
    ...
    char *argv[];
{
     ...
     */
    if (argc != 3) {
	fprintf(stderr, "usage: %s <waterTHAxyz_file> <ascii_file>\n",
		argv[0]);
	exit(0);
    }
	...
	for(j=0;j<3;j++){
	    i=k*3+j;
	    fprintf(fp,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",i,H2OID[j],fx[i],fy[i],fz[i],(j==0?0026:0004),k*3,(j==0?2:0),(j==0?3:0));
	}
    }
//print nitrogen parent
    i=waters*3;
    fprintf(fp,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",i,NID,fx[i],fy[i],fz[i],0026,waters*3,76,77);
    for(k=0;k<molec;k++){
	for(j=0;j<per;j++){
	    i=1+waters*3+per*k+j;

	    fprintf(fp,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",i,THAID[j],fx[i],fy[i],fz[i],0004,waters*3,0,0);
	}
    }
    /*
