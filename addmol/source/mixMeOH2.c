/*
 *	mixMeOH2.c:	Replace a number of CH3CN molecules between z1 and z2
 *		by the same  number of CH3OH molecules. Allows for an input file that already 
 *		has MeOH molecules present. 
 *	To call:	mixMeOH <binary_file> <ascii_file> <preexistingCH3OH> <nCH3OH> <initial nCH3CN> <z1> <z2>
 *
 */
...
main (argc, argv)
	...
	char	*argv[];
{
	int	i,k,oldCH3OH,nCH3OH,nCH3CN,actualM,nA,list[1023];
 ...
 */
	if (argc != 8) {
		fprintf(stderr, "usage: %s <binary_file> <ascii_file> <preexistingCH3OH> <nCH3OH> <nCH3CN> <z1> <z2>\n",
			argv[0]);
		exit(0);
	}
oldCH3OH = atoi(argv[3]);
nCH3OH = atoi(argv[4]);
nCH3CN = atoi(argv[5]);
z1 = atof(argv[6]);
z2 = atof(argv[7]);
...
nA = 0;
actualM = 0;
/* print pre-existing methanols */
for (i=0; i<3*oldCH3OH; i++) {
                fprintf(fp,
...
for (i=3*oldCH3OH; i< 3*(oldCH3OH + nCH3CN); i += 3) {
   if (pos[i].fz > z1 && pos[i].fz < z2 && actualM < nCH3OH){/* replace this CH3CN*/
	selectCH3OH(&pos[i],pM);
	fprintf(fp,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",(oldCH3OH+actualM)*3,O_type,pM[0].fx,pM[0].fy,pM[0].fz,atom[i].flags,(oldCH3OH+actualM)*3,atom[i].param1,atom[i].param2);
	fprintf(fp,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",(oldCH3OH+actualM)*3+1,H_type,pM[1].fx,pM[1].fy,pM[1].fz,atom[i+1].flags,(oldCH3OH+actualM)*3,atom[i+1].param1,atom[i+1].param2);
	fprintf(fp,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",(oldCH3OH+actualM)*3+2,M_type,pM[2].fx,pM[2].fy,pM[2].fz,atom[i+2].flags,(oldCH3OH+actualM)*3,atom[i+2].param1,atom[i+2].param2);
	actualM++;
    }
...
for (i=0; i<nA; i++) {
	k = list[i];
	fprintf(fp,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",(oldCH3OH+actualM)*3+i*3,atom[k].type,pos[k].fx,pos[k].fy,pos[k].fz,atom[k].flags,(oldCH3OH+actualM)*3+i*3,atom[k].param1,atom[k].param2);
	fprintf(fp,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",(oldCH3OH+actualM)*3+i*3+1,atom[k+1].type,pos[k+1].fx,pos[k+1].fy,pos[k+1].fz,atom[k+1].flags,(oldCH3OH+actualM)*3+i*3,atom[k+1].param1,atom[k+1].param2);
	fprintf(fp,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",(oldCH3OH+actualM)*3+i*3+2,atom[k+2].type,pos[k+2].fx,pos[k+2].fy,pos[k+2].fz,atom[k+2].flags,(oldCH3OH+actualM)*3+i*3,atom[k+2].param1,atom[k+2].param2);
}

for (i=(oldCH3OH + nCH3CN)*3; i<natoms; i++) {
                fprintf(fp,
"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",
i,atom[i].type,pos[i].fx,pos[i].fy,pos[i].fz,atom[i].flags,atom[i].parent,atom[i].param1,atom[i].param2);
        }
