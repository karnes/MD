/*
 *	mixMCN3.c:	Replace a number of CH3OH molecules between z1 and z2
 *		by the same  number of CH3CN molecules. Limits replaced CH3CN to "center" of box.
 *	To call:	mixMCN3 <binary_file> <ascii_file> <nCH3CN> <initial nCH3OH> <z1> <z2>
 *
 */
...
main (argc, argv)
	...
	char	*argv[];
{
	int	i,k,nCH3CN,nCH3OH,actualM,nA;//list[1023];
 ...
 */
	if (argc != 7) {
		fprintf(stderr, "usage: %s <binary_file> <ascii_file> <nCH3CN> <nCH3OH> <z1> <z2>\n",
			argv[0]);
		exit(0);
	}
nCH3CN = atoi(argv[3]);
...
z2 = atof(argv[6]);
	if (nCH3CN != 1) {
		fprintf(stderr, "%s currently only works for nCH3CN = 1. Exiting...\n",
			argv[0]);
		exit(0);
	}
//O_type = 45;
...
actualM = 0;
//actualA = 0; 
//fprintf(stderr,"before search for MeOH\n");
for (i=0; i<nCH3OH*3; i += 3) {
   if (!(pos[i].fz > z1 && pos[i].fz < z2 && (pos[i].fx*pos[i].fx+pos[i].fy*pos[i].fy) < 25.0 && nA < nCH3CN)){/* don't replace this CH3OH, print CH3OH*/
	k=i;
	fprintf(fp,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",3*actualM,atom[k].type,pos[k].fx,pos[k].fy,pos[k].fz,atom[k].flags,actualM*3,atom[k].param1,atom[k].param2);
	fprintf(fp,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",3*actualM+1,atom[k+1].type,pos[k+1].fx,pos[k+1].fy,pos[k+1].fz,atom[k+1].flags,actualM*3,atom[k+1].param1,atom[k+1].param2);
	fprintf(fp,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",3*actualM+2,atom[k+2].type,pos[k+2].fx,pos[k+2].fy,pos[k+2].fz,atom[k+2].flags,actualM*3,atom[k+2].param1,atom[k+2].param2);
	actualM++;
    }
    else {/* make this CH3OH a CH3CN*/
	k=i;
	fprintf(stderr,"Selected MeOH molecule:\n");
	fprintf(stderr,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",k,atom[k].type,pos[k].fx,pos[k].fy,pos[k].fz,atom[k].flags,actualM*3,atom[k].param1,atom[k].param2);
	fprintf(stderr,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",k+1,atom[k+1].type,pos[k+1].fx,pos[k+1].fy,pos[k+1].fz,atom[k+1].flags,actualM*3,atom[k+1].param1,atom[k+1].param2);
	fprintf(stderr,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",k+2,atom[k+2].type,pos[k+2].fx,pos[k+2].fy,pos[k+2].fz,atom[k+2].flags,actualM*3,atom[k+2].param1,atom[k+2].param2);
	selectCH3OH(&pos[i],pM);
	nA++;
    }
}
if (nCH3OH - actualM < nCH3CN){
	fprintf(stderr,"incomplete replacemnt of CH3OH. nCH3CN = %d\n",nA);
	fprintf(stderr," z1 = %f, z2 = %f, probably should increase z2-z1\n",z1,z2);
}
fprintf(stderr,"nA = %d\n",nA);
for (i=0; i<nA; i++) {
	fprintf(fp,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",actualM*3,C_type,pM[0].fx,pM[0].fy,pM[0].fz,atom[i].flags,actualM*3,atom[i].param1,atom[i].param2);
	fprintf(fp,"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",actualM*3+1,N_type,pM[1].fx,pM[1].fy,pM[1].fz,atom[i+1].flags,actualM*3,atom[i+1].param1,atom[i+1].param2);
...
tripd v1,v2;
double b;
pm[0].fx = pA[0].fx; pm[0].fy = pA[0].fy; pm[0].fz = pA[0].fz; /*C takes O's position*/
...
/* The N and the CH3 groups are placed along the two old bonds at the right bond length*/
pm[1].fx = pm[0].fx + v1.fx *rCN;
pm[1].fy = pm[0].fy + v1.fy *rCN;
pm[1].fz = pm[0].fz + v1.fz *rCN;
pm[2].fx = pm[0].fx + v2.fx *rMC;
pm[2].fy = pm[0].fy + v2.fy *rMC;
pm[2].fz = pm[0].fz + v2.fz *rMC;
}
