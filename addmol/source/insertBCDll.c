/*
 *	insertBCDll.c:	reads xyz positions of BCD
 *                      and insert in center of liquid-liquid box.
 *
 *	To call:	insBCDll <l/l binary> <BCD binary> <nwater> <BCD_CoM_z> <ascii_file>
 *
 */
#include "typedefs.h"
#include "atomtypes.h"
...
main (argc, argv)
	...
	char	*argv[];
{
	int	i,j,m,k,l,nw,nBrO,nWR,nBR,inC[9999], inB[9999];
 ...
 */
	if (argc != 6) {
		fprintf(stderr, "usage: %s <liq-liq binary> <BCD binary> <nwater> <BCD_CoM_z> <ascii_file>\n",
			argv[0]);
		exit(0);
	}
...
for(i=0;i<nBrO;i++){
   m = nw*3 + i*BrOs;
   inB[i] = 0; /* assume BrO is outside BCD radius */
   for(j=natoms2-147;j<natoms2;j++){
      for(k=0;k<BrOs;k++){
...
*/	  
fprintf(stderr," %d water molecules will be removed to make room for the bCD. %d total to remove.\n",nWR,rmW);
fprintf(stderr," %d Br-octane molecules will be removed to make room for the bCD. %d total to remove.\n",nBR,rmB);
/*
...
//j = natoms - nWR * 3;
for (k=natoms2-147; k<natoms2; k++) {/*loop over all the ETH molecules*/
		fprintf(fpo,
"%4d %3d  %12.5f  %12.5f  %12.5f   %04o  %4d %4d %4d\n",
k+i-(natoms2-147),atom2[k].type,pos2[k].fx,pos2[k].fy,pos2[k].fz,atom2[k].flags,atom2[k].parent + i-(natoms2-147),atom2[k].param1,atom2[k].param2);
}
/*
