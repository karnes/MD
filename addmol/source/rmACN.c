/*
 *	rmACN.c:	remove all the ACN molecules between z1 and z2 (z1 < z2) leaving only nA
 *	To call:	rmACN <binary_file> <ascii_file> <initial nCH3CN> <z1> <z2> <nA>
 *
 */
 ...
 */
	if (argc != 7) {
		fprintf(stderr, "usage: %s <binary_file> <ascii_file> <nCH3CN> <z1> <z2> <nA>\n",
			argv[0]);
		exit(0);
	}
nCH3CN = atoi(argv[3]);
...
for (i=0; i<nCH3CN*3; i += 3) {
   if (pos[i].fz < z1 || pos[i].fz > z2){/* keep this CH3CN*/
...
if (n != nA){
fprintf(stderr,"error selecting nM. nM must match the actual number of CH3CN molecules between z1 and z2 in the input file.\n");
exit(1);
}
