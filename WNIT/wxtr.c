...
if (initQ == 1 ) {
	...
	poshist[0].fx=pos[ion].fx;
	poshist[0].fy=pos[ion].fy;
	poshist[0].fz=pos[ion].fz;
	for(k=0;k<tcData;k++){
		for(i=0;i<3;i++){
...
for(k=0;k<tdpoint;k++){
	dip.fx = pos[ion].fx - poshist[k].fx;
	dip.fy = pos[ion].fy - poshist[k].fy;
	dip.fz = pos[ion].fz - poshist[k].fz;
	pimage(&dip);
...
poshist[tdpoint].fx = pos[ion].fx;
poshist[tdpoint].fy = pos[ion].fy;
poshist[tdpoint].fz = pos[ion].fz;
tdpoint++;
