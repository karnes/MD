...
/*
	fprintf(stderr,"befor shake(%c)\n",type);
	fprintf(stderr,"%f %f %f %f %f %f\n,",pos[0].fx,pos[0].fy,pos[0].fz,vel[0].fx,vel[0].fy,vel[0].fz);
	fprintf(stderr,"%f %f %f %f %f %f\n,",pos[1].fx,pos[1].fy,pos[1].fz,vel[1].fx,vel[1].fy,vel[1].fz);
*/
	shake_a(type);
/*
	fprintf(stderr,"after shake(%c)\n",type);
	fprintf(stderr,"%f %f %f %f %f %f\n,",pos[0].fx,pos[0].fy,pos[0].fz,vel[0].fx,vel[0].fy,vel[0].fz);
	fprintf(stderr,"%f %f %f %f %f %f\n,",pos[1].fx,pos[1].fy,pos[1].fz,vel[1].fx,vel[1].fy,vel[1].fz);
*/
}
...
{ 
tripd rij, vij,vcm;
double rv, factor,kin,oldkin,Tcm;
rij.fx = pos[i].fx - pos[j].fx;
rij.fy = pos[i].fy - pos[j].fy;
rij.fz = pos[i].fz - pos[j].fz;
mvimage(&rij);
...
vcm.fx = (mass[i]*vel[i].fx + mass[j]*vel[j].fx)/(mass[i]+mass[j]);
vcm.fy = (mass[i]*vel[i].fy + mass[j]*vel[j].fy)/(mass[i]+mass[j]);
vcm.fz = (mass[i]*vel[i].fz + mass[j]*vel[j].fz)/(mass[i]+mass[j]);
Tcm = (mass[i]+mass[j])*(vcm.fx*vcm.fx + vcm.fy*vcm.fy + vcm.fz*vcm.fz);
oldkin =
mass[i]*(sq(vel[i].fx) + sq(vel[i].fy) +sq(vel[i].fz))+
mass[j]*(sq(vel[j].fx)+ sq(vel[j].fy) +sq(vel[j].fz));
...
kin = mass[i]*(sq(vel[i].fx) + sq(vel[i].fy) +sq(vel[i].fz))+
      mass[j]*(sq(vel[j].fx) + sq(vel[j].fy) +sq(vel[j].fz));
factor = sqrt((oldkin-Tcm)/(kin-Tcm));
vel[i].fx = (vel[i].fx - vcm.fx)*factor + vcm.fx;
vel[i].fy = (vel[i].fy - vcm.fy)*factor + vcm.fy;
vel[i].fz = (vel[i].fz - vcm.fz)*factor + vcm.fz;
vel[j].fx = (vel[j].fx - vcm.fx)*factor + vcm.fx;
vel[j].fy = (vel[j].fy - vcm.fy)*factor + vcm.fy;
vel[j].fz = (vel[j].fz - vcm.fz)*factor + vcm.fz;
}
