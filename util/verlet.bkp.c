...
/****** get force (-dV/dr) ******/
	getforce(pos, force);
//fprintf(stderr,"got forces, back in verlet.c\n");
/*	if (!ngo) wfsolute(stderr,1);*/
	for 	(i = 0; i < natoms; i++)
		if 	(atom[i].flags & A_FIXED)
			force[i].fx = force[i].fy = force[i].fz = 0.;
//fprintf(stderr," verlet - line 107.c\n");

	if	(!fix_T)	/* normal verlet algorithm */
		{
//fprintf(stderr," verlet - norm verlet.c\n");
		if	(ngo)	/* not returning yet */
			{
			for	(i = 0; i < natoms; i++)
				{
//fprintf(stderr," verlet - line 115.c\n");
/****** just calculate accelerations ******/
			...
			{
//fprintf(stderr," verlet.c - will be returning\n");
			for	(i = 0; i < natoms; i++)
				{
/****** calculate true velocities and accelerations ******/
//fprintf(stderr," verlet.c - in velocity calc loop. i = %d\n",i);
//fprintf(stderr," verlet.c - vel[0].fx = %f imass[0] = %f force[0].fx = %f tvel[0] = %f\n",vel[i].fx, imass[i],force[i].fx,tvel[0].fx);
				tvel[i].fx = vel[i].fx +
...
//fprintf(stderr," verlet.c - before constrain\n");
		...
		is2 = 1./ (s*s);
		alpha = s * s / (s + h1*vs);
//fprintf(stderr," verlet - line 155.c\n");
...
//fprintf(stderr,"verlet.c: calculated kinetic energy\n");
...
//fprintf(stderr,"verlet.c: calculated elapsed time\n");
...
//fprintf(stderr,"verlet.c: incremented velocities\n");
