#include	<md.h>
#include	<water.h>
#define HBondr2  12.25
#define HBond_a 0.766

waterForce(pos, force, na)
tripd	*pos;
tripd	*force;
int	na;
{
int	i, j, k, l, m, n, hbflag, doublehb;
double	r2, dedr, delfx, delfy, delfz, ROH;
double		eg, s, sp;
tripd		frc, image, sdl, f[6], rOH;
double		spc(), sqrt();
double		watts();
...
/*****	Determine image vector ******/
				...
				r2 = delfx*delfx + delfy*delfy + delfz*delfz;
/* check the angle between the OH bond of the i'th water and the OH
 * vector connecting the O of the j'th water and one of the two H
 * of th i'th water if hblag is on*/
if (hbflag == 1){
	if (m>0 && n ==0){/* looking at possible H participating in bonding*/
		rOH.fx = pos[k].fx-pos[k+m].fx;	
		rOH.fy = pos[k].fy-pos[k+m].fy;	
		rOH.fz = pos[k].fz-pos[k+m].fz;	
		ROH = sqrt(rOH.fx*rOH.fx+rOH.fy*rOH.fy+rOH.fz*rOH.fz);
		if ((delfx*rOH.fx+delfy*rOH.fy+delfz*rOH.fz)/(ROH*sqrt(r2)) > HBond_a){
			hbmat[j][i] = 1;
			hbmat[i][j] = 1;
			doublehb++;
		}
	...
	if (m==0 && n >0){/* looking at possible H participating in bonding*/
		rOH.fx = pos[l+n].fx-pos[l].fx;	
		rOH.fy = pos[l+n].fy-pos[l].fy;	
		rOH.fz = pos[l+n].fz-pos[l].fz;	
		ROH = sqrt(rOH.fx*rOH.fx+rOH.fy*rOH.fy+rOH.fz*rOH.fz);
		if ((delfx*rOH.fx+delfy*rOH.fy+delfz*rOH.fz)/(ROH*sqrt(r2)) > HBond_a){
			hbmat[j][i] = 1;
			hbmat[i][j] = 1;
...
/*
if (doublehb > 1)
fprintf(stderr,"warning: mulitple hydrogen bond between molecules %d %d\n",i,j);
*/
}
/*****	Get (1/r dV/dr) ******/
...
if (m == 0){ /*Oxygen*/
	if (n == 0){ /*O-O*/
		...
		*eg += OO_EPSIL4*ir6*(ir6-1.) + EC;
		...
		i = 20.0/ir;
		if( i >= 0 && i < 500 ) waterRdf[i] += 1.0;
		else { fprintf(stderr,"waterForce.c: waterRdf index out of bounds, i = %d\n", i); }
	}
	else { /*O-H*/
		EC = OQ*HQ*ir/E2;
		*eg += EC;
