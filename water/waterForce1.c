...
int	na;
{
int	i, j, k, l, m, n, ih, hbflag, hbcounter,doublehb;
...
for	(i = 0; i < na; i++)
	...
	hbcounter = 0;
		...
		for (ih = 0; ih < 10; ih++) hbmat[ih][i] = 0;
...
if (hbflag == 1){
	if (m>0 && n ==0){/* looking at possible H participating in bonding*/
		...
		if ((delfx*rOH.fx+delfy*rOH.fy+delfz*rOH.fz)/(ROH*sqrt(r2)) > 0.866){
			hbmat[hbcounter][i] = j;
			doublehb++;
			hbcounter++;
		}
...
/*****	Resolve forces on atoms.  ******/
	...
	hbmat[9][i] = hbcounter; /* store the number of Hbonds*/
	}
}
#ifdef NEWSPC
#define	RE	1.000
