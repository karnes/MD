...
/*****	Get intramolecular forces for i'th water.  ******/
	...
	if(action[1]!='E'){
	  if(tc%dataRatex==0&&tc>0){
	   dip.fx = (pos[k+1].fx + pos[k+2].fx)/2 - pos[k].fx;
	   dip.fy = (pos[k+1].fy + pos[k+2].fy)/2 - pos[k].fy;
	   dip.fz = (pos[k+1].fz + pos[k+2].fz)/2 - pos[k].fz;
	   r = sqrt(dip.fx*dip.fx + dip.fy*dip.fy + dip.fz*dip.fz);
	   dip.fx /= r;
	   dip.fy /= r;
	   dip.fz /= r;
	   dipVec[i][tc/dataRatex - 1].fx = dip.fx;
	   dipVec[i][tc/dataRatex - 1].fy = dip.fy;
	   dipVec[i][tc/dataRatex - 1].fz = dip.fz;
	 }
	}
